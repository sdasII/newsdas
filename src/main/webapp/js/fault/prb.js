...
var ajax_url = "";// 请求地址
var work_url= ctx + "/fault/getworkprb";
var date_attr = "";// 属性
var dom_id = "";// dom_id
var line_color = "";// 颜色
var starttime="";
...
var work_start="";
var work_end="";
var prbUrl = ctx + "/fault/prb";
var switchUrl = ctx + "/fault/switch";
var lastDayUrl = ctx + "/indexalarm/getLastDay";
$(function() {
	//曲线时间选择
	...
	$(".workPicker").click(function() {
		...
		$(this).parent().children(":last").css("display", "none");
			...
			if(work_date!=null){
				if ($("#topTabs").find(".active").find("a").html().indexOf("PRB利用率") > -1) {
					work_data("#work_table","/newsdas/fault/getworkprb",work_date,work_start,work_end);
				} else {
					work_data("#work_table_switch","/newsdas/fault/getworkswitch",work_date,work_start,work_end);
				}
			}
		});
	...
	$("#topTabs").find("li").click(function(){
		...
		if (title.indexOf("PRB利用率")>-1) {
			work_url= ctx + "/fault/getworkprb";
			PRBCharts(date_value);
			work_data("#work_table","/newsdas/fault/getworkprb",work_date,work_start,work_end);
			alarm_data("#alarm_table",0);
		}else{
			work_url=ctx+"/fault/getworkswitch";
			switchCharts(date_value);
			work_data("#work_table_switch","/newsdas/fault/getworkswitch",work_date,work_start,work_end);
	...
	$("#tab").find("li").click(function() {
				...
				if (title == "关联指标对比") {
					...
					ajax_url = ctx + "/fault/getprbothers";
					if (title == "用户面最大激活UE数") {
						date_attr = "uecounts";
						dom_id = "#UE";
						line_color = "#2EC7C9";
	...
	$("#tab_switch").find("li").click(
			function() {
				...
				if (title == "关联指标对比") {
					...
					ajax_url = ctx + "/fault/getswitchothers";
